...
class Daily_Log(customtkinter.CTkFrame):
    ...
    def upperDisplay(self):
        ...
        self.date_string = strftime('%Y/%m/%d')
        ...
        self.RTLabel = customtkinter.CTkLabel(self.logDetail, text="Resting Time",font = ("v",18),justify = "center",padx = 50,pady=10)
    ...
    def save(self):
        ...
        if (self.checkDates() or self.checkDInput() or self.checkRInput() or self.checkTimes()) == True:
            self.cur.execute("INSERT OR IGNORE INTO logs VALUES (?, ?, ?)", ["word", 2,3])
            self.con.commit()
